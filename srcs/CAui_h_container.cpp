...
Init_h_container	CAui_h_container::not_use;

CAui_h_container::CAui_h_container()
{
	pos.x = 0;
	pos.y = 0;
	size.width = 0;
...
t_size	CAui_h_container::draw_child(float pos_x, float pos_y)
{
	t_size	rt;
	t_size	tmp;
	int	i;
	...
	while (++i < (int)content.size())
	{
		...
		rt.height = (rt.height > content[i]->size.height) ? rt.height : tmp.height;
		rt.width += tmp.width;
		pos_x += tmp.width;
	}
	return (rt);
...
void	CAui_h_container::mouse_button_callback(int window, int action, int mods)
{

}

void	CAui_h_container::cursor_position_callback(int status, double xpos, double ypos)
{

}

CAui	*CAui_h_container::why(float x, float y)
{
	int	i;
	...
	while (++i <  (int)content.size())
	{
		...
		x -= content[i]->size.width + content[i]->pos.x;
	}
	return (0);
}
